...
cls = "os.system('cls')"
line_index = 2
input_file = easygui.fileopenbox(title="Select dictionary file",filetypes=['*.unshare'], default='*.unshare')
os.chdir(cwd_temp_download)
linecache.clearcache()
info = linecache.getline(input_file, 1).rstrip('\n')
output_directory = easygui.diropenbox(title="Save as")
exec(cls)
info = info.split(";")
if info[0] != "head":
    ...
    if input("Continue? (y/n): ").lower() == "y":
        ...
        merge = Merge(cwd_temp_download, output_directory, info[1])
